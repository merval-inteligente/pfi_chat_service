...
# Respuestas demo
async def get_ai_response_demo(message: str, user_id: str) -> str:
...
⚠️ **Perfil**: Solo para inversores con tolerancia al riesgo soberano alto."""
    ...
    if any(word in message_lower for word in ["merval", "mercado", "índice"]):
        ...
        return f"""¡Hola {user_id}! 👋 Soy tu **asistente financiero argentino**.
...
async def send_message(chat_msg: ChatMessage):
    ...
    try:
        # Generar respuesta
        ai_response = await get_ai_response_demo(chat_msg.message, chat_msg.user_id)
